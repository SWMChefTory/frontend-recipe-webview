...
export const useCarouselController = (recipeData: RecipeData, sliderRef: React.RefObject<Slider>) => {
  const {
    // sliderRef,
    // currentStep,
    // slickSettings,
  ...
  const timelineStarts = useMemo(() => converToTimelineStarts(recipeData), [recipeData]);

  const totalSteps = useMemo(() => count(recipeData), [recipeData]);
  ...
  const carouselControls = useMemo(
    ...
    [totalSteps, timelineStarts, goToNext, goToPrevious, goToStep],
  );
